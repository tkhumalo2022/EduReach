...
The expected Wix CMS collection IDs are `Articles`, `Ebooks`, `DownloadableResources`, `WorkshopAlbums`, `GalleryAlbums` and `Categories`. Wix's built-in draft/publish controls manage visibility; do not add a separate manual status field.
